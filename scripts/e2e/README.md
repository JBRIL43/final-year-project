...
- Node.js installed
- Access to the target PostgreSQL database (set `SUPABASE_DATABASE_URL` or `DATABASE_URL` to point to it)

## Run the test

```bash
SUPABASE_DATABASE_URL="postgres://user:pass@host:5432/dbname?sslmode=require" node scripts/e2e/withdrawal_workflow_db_test.js
```
